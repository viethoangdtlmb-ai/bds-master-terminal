        ...
        function newSession() {
            if (confirm('Bắt đầu buổi khám mới? Dữ liệu hiện tại sẽ được xóa.')) {
                ...
                localStorage.removeItem('aa_portfolio');
                localStorage.removeItem('aa_strategy_cart');
                location.reload();
            }
        }
        ...
        async function loadMarketData() {
            ...
            if (md) {
                ...
                stamp.textContent = `📡 Dữ liệu: ${md.districts.length} khu vực`;
        ...
        function populateDistrictDropdowns() {
            ...
            ['f-district', 'qs-district', 'cn-district'].forEach(id => {
...
&nbsp;|&nbsp; ROE/vốn: <strong style="color:${roeAnn >= 15 ? 'var(--emerald)' : roeAnn >= 8 ? 'var(--yellow)' : 'var(--red)'}">${roeAnn.toFixed(1)}%/năm</strong>
...
`;
        ...
        // ── Templates ─────────────────────────────────────────────────
                                ...
                                const TEMPLATES = {
            // ── Chung cư Hoàng Mai (75m²) ──
            // Cost 2020: 2.5T. Loan: 1.5T (25 yrs). Passed: 6 yrs -> Debt: 1.14T.
            chungcu: { name: 'Căn Hộ Hoàng Mai', type: 'chung-cu', district: 'Hoàng Mai', area: 75, year: 2020, cost: 2.9, market: 7.4, goal: 'cho-thue', loanpct: 60, debt: 1.31, rate: 8.0, prefmonths: 0, floatrate: 10.5, grace: 0, loanterm: 25, rentstatus: 'dang-thue', rent: 15, mgmt: 1.5, maint: 5, delivery: 0, rentExpected: 0 },

            // ── Chung cư Cầu Giấy (100m²) ──
            // Cost 2021: 5.0T. Loan: 2.5T (20 yrs). Passed: 5 yrs -> Debt: 1.88T.
            shophouse: { name: 'Căn Hộ Cầu Giấy', type: 'chung-cu', district: 'Cầu Giấy', area: 100, year: 2021, cost: 5.8, market: 14.1, goal: 'cho-thue', loanpct: 50, debt: 2.16, rate: 8.0, prefmonths: 0, floatrate: 10.5, grace: 0, loanterm: 20, rentstatus: 'dang-thue', rent: 23, mgmt: 5, maint: 8, delivery: 0, rentExpected: 0 },

            // ── Biệt Thự Tây Hồ (200m²) ──
            // Cost 2018: 32T. Loan: 12.8T (15 yrs). Passed: 8 yrs -> Debt: 5.97T.
            bietthu: { name: 'Biệt Thự Tây Hồ', type: 'biet-thu', district: 'Tây Hồ', area: 200, year: 2018, cost: 36.8, market: 71.4, goal: 'tu-o', loanpct: 40, debt: 6.87, rate: 8.5, prefmonths: 0, floatrate: 10.5, grace: 0, loanterm: 15, rentstatus: 'trong', rent: 0, mgmt: 8, maint: 35, delivery: 0, rentExpected: 0 },

            // ── Đất Nền Hoài Đức (100m²) ──
            // Cost 2022: 9.0T. Loan: 4.5T (15 yrs). Passed: 4 yrs -> Debt: 3.3T.
            datnen: { name: 'Đất Nền Hoài Đức', type: 'dat-nen', district: 'Hoài Đức', area: 100, year: 2022, cost: 10.4, market: 18.9, goal: 'tang-gia', loanpct: 50, debt: 3.79, rate: 9.0, prefmonths: 0, floatrate: 11.0, grace: 0, loanterm: 15, rentstatus: 'trong', rent: 0, mgmt: 0, maint: 0, delivery: 0, rentExpected: 0 },

            // ── Biệt Thự Long Biên (100m²) ──
            // Cost 2019: 12.0T. Loan: 8.4T (20 yrs). Passed: 7 yrs -> Debt: 5.46T.
            nghiduong: { name: 'Biệt Thự Long Biên', type: 'biet-thu', district: 'Long Biên', area: 100, year: 2019, cost: 13.8, market: 31.6, goal: 'tu-o', loanpct: 70, debt: 6.28, rate: 8.5, prefmonths: 0, floatrate: 10.5, grace: 0, loanterm: 20, rentstatus: 'trong', rent: 0, mgmt: 5, maint: 10, delivery: 0, rentExpected: 0 },

            // ── Nhà Phố Đống Đa (150m²) ──
            // Cost 2015: 25.0T. Loan: 12.5T (15 yrs). Passed: 11 yrs -> Debt: 3.34T.
            vanphong: { name: 'Nhà Phố Đống Đa', type: 'nha-pho', district: 'Đống Đa', area: 150, year: 2015, cost: 28.7, market: 51.7, goal: 'cho-thue', loanpct: 50, debt: 3.84, rate: 8.5, prefmonths: 0, floatrate: 10.5, grace: 0, loanterm: 15, rentstatus: 'dang-thue', rent: 138, mgmt: 10, maint: 20, delivery: 0, rentExpected: 0 },

            // ── Nhà Phố Hai Bà Trưng (80m²) ──
            // Cost 2015: 10.0T. Loan: 2.0T (15 yrs). Passed: 11 yrs -> Debt: 0.53T.
            nhapho: { name: 'Nhà Phố Hai Bà Trưng', type: 'nha-pho', district: 'Hai Bà Trưng', area: 80, year: 2015, cost: 11.5, market: 26, goal: 'cho-thue', loanpct: 20, debt: 0.61, rate: 8.5, prefmonths: 0, floatrate: 10.5, grace: 0, loanterm: 15, rentstatus: 'dang-thue', rent: 57, mgmt: 5, maint: 10, delivery: 0, rentExpected: 0 },

            // ── CHDV Thanh Xuân (100m²) ──
            // Cost 2022: 15.0T. Loan: 7.5T (12 yrs). Passed: 4 yrs -> Debt: 5.0T.
            chdv: { name: 'CHDV Thanh Xuân', type: 'chdv', district: 'Thanh Xuân', area: 100, year: 2022, cost: 17.2, market: 31.7, goal: 'cho-thue', loanpct: 50, debt: 5.75, rate: 9.0, prefmonths: 0, floatrate: 10.5, grace: 0, loanterm: 12, rentstatus: 'dang-thue', rent: 115, mgmt: 10, maint: 20, delivery: 0, rentExpected: 0 },

            // ── Căn Hộ Cao Cấp Ba Đình (120m²) ──
            // Cost 2024: 18.0T. Loan: 7.2T (20 yrs). Passed: 2 yrs -> Debt: 6.48T.
            chungcuvip: { name: 'Căn Hộ Ba Đình', type: 'chung-cu', district: 'Ba Đình', area: 120, year: 2024, cost: 20.7, market: 25.4, goal: 'tu-o', loanpct: 40, debt: 7.45, rate: 7.5, prefmonths: 12, floatrate: 10.0, grace: 0, loanterm: 20, rentstatus: 'trong', rent: 0, mgmt: 6, maint: 15, delivery: 0, rentExpected: 0 },

            // ── Đất Nền Thanh Trì (80m²) ──
            // Cost 2021: 5.0T. Loan: 1.5T (10 yrs). Passed: 5 yrs -> Debt: 0.75T.
            datvuon: { name: 'Đất Nền Thanh Trì', type: 'dat-nen', district: 'Thanh Trì', area: 80, year: 2021, cost: 5.8, market: 15, goal: 'tang-gia', loanpct: 30, debt: 0.86, rate: 8.5, prefmonths: 0, floatrate: 10.5, grace: 0, loanterm: 10, rentstatus: 'trong', rent: 0, mgmt: 3, maint: 10, delivery: 0, rentExpected: 0 },

            // ── Vinhomes Smart City Nam Từ Liêm (55m²) ──
            // Cost 2021: 2.2T. Loan: 1.54T (35 yrs). Passed: 5 yrs -> Debt: 1.32T.
            smartcity: { name: 'Vinhomes Smart City', type: 'chung-cu', district: 'Nam Từ Liêm', area: 55, year: 2021, cost: 2.5, market: 5.4, goal: 'cho-thue', loanpct: 70, debt: 1.52, rate: 8.0, prefmonths: 0, floatrate: 10.5, grace: 24, loanterm: 35, rentstatus: 'dang-thue', rent: 16, mgmt: 1.5, maint: 5, delivery: 0, rentExpected: 0 },

            // ── Liền kề Vinhomes Cổ Loa Đông Anh (63m² đất) ──
            // Cost 2025: 20T. Loan: 14.0T. HTLS 18 months, no principal paid -> Debt: 14.0T.
            vincoloa: { name: 'Liền kề Vinhomes Cổ Loa', type: 'biet-thu', district: 'Đông Anh', area: 63, year: 2025, cost: 23, market: 25.3, goal: 'tang-gia', loanpct: 70, debt: 16.1, rate: 0, prefmonths: 18, floatrate: 10.5, grace: 18, loanterm: 20, rentstatus: 'trong', rent: 0, mgmt: 2, maint: 0, delivery: 0, rentExpected: 0 },

            // ── Ocean Park 1 Gia Lâm (43m²) ──
            // Cost 2020: 1.6T. Loan: 1.12T (35 yrs). Passed: 6 yrs -> Debt: 0.93T.
            ocp1: { name: 'Ocean Park 1 Gia Lâm', type: 'chung-cu', district: 'Gia Lâm', area: 43, year: 2020, cost: 1.8, market: 3.3, goal: 'cho-thue', loanpct: 70, debt: 1.07, rate: 8.0, prefmonths: 0, floatrate: 10.5, grace: 0, loanterm: 35, rentstatus: 'dang-thue', rent: 10, mgmt: 1, maint: 5, delivery: 0, rentExpected: 0 },

            // ── Shophouse Ocean Park 2 Văn Giang (48m²) ──
            // Cost 2023: 7.5T. Loan: 5.25T (20 yrs). HTLS 2 yrs -> 1 yr paid -> Debt: 4.99T.
            ocp2: { name: 'Shophouse Ocean Park 2', type: 'shophouse', district: 'Văn Giang (HY)', area: 48, year: 2023, cost: 8.6, market: 7.4, goal: 'tu-o', loanpct: 70, debt: 5.74, rate: 0, prefmonths: 24, floatrate: 10.5, grace: 24, loanterm: 20, rentstatus: 'trong', rent: 0, mgmt: 1.5, maint: 0, delivery: 0, rentExpected: 0 },

            // ── Căn hộ Hà Đông (85m²) ──
            // Cost 2020: 2.5T. Loan: 1.25T (20 yrs). Passed: 6 yrs -> Debt: 0.88T.
            hadong: { name: 'Căn hộ Hà Đông', type: 'chung-cu', district: 'Hà Đông', area: 85, year: 2020, cost: 2.9, market: 7, goal: 'tu-o', loanpct: 50, debt: 1.01, rate: 8.5, prefmonths: 0, floatrate: 10.5, grace: 0, loanterm: 20, rentstatus: 'trong', rent: 0, mgmt: 1.5, maint: 5, delivery: 0, rentExpected: 0 },

            // ── Ngoại Giao Đoàn Bắc Từ Liêm (105m²) ──
            // Cost 2019: 4.5T. Loan: 2.25T (20 yrs). Passed: 7 yrs -> Debt: 1.46T.
            bactuliem: { name: 'Ngoại Giao Đoàn Bắc Từ Liêm', type: 'chung-cu', district: 'Bắc Từ Liêm', area: 105, year: 2019, cost: 5.2, market: 11.7, goal: 'cho-thue', loanpct: 50, debt: 1.68, rate: 7.5, prefmonths: 0, floatrate: 10.5, grace: 0, loanterm: 20, rentstatus: 'dang-thue', rent: 32, mgmt: 2, maint: 8, delivery: 0, rentExpected: 0 },

            // ── Mỹ Đình Nam Từ Liêm (75m²) ──
            // Cost 2018: 2.5T. Loan: 1.5T (20 yrs). Passed: 8 yrs -> Debt: 0.9T.
            namtuliem: { name: 'Căn Hộ Mỹ Đình', type: 'chung-cu', district: 'Nam Từ Liêm', area: 75, year: 2018, cost: 2.9, market: 7.4, goal: 'cho-thue', loanpct: 60, debt: 1.03, rate: 8.0, prefmonths: 0, floatrate: 10.5, grace: 0, loanterm: 20, rentstatus: 'dang-thue', rent: 22, mgmt: 1.8, maint: 6, delivery: 0, rentExpected: 0 }
        ...
        function loadTemplate(key, ev) {
            ...
            document.querySelectorAll('#templates-grid .btn').forEach(b => b.classList.remove('btn-primary'));
...
` : ''}
        ...
        function renderAssetList() {
            ...
            try {
                ...
                if (btnD) btnD.style.display = PORTFOLIO.length > 0 ? 'inline-flex' : 'none';
                ...
                if (btnQP) btnQP.style.display = PORTFOLIO.length > 0 ? 'inline-flex' : 'none';
                                ...
                                // Ẩn/Hiện onboarding-guide và quick-scan-banner dựa trên số lượng tài sản
                const guide = document.getElementById('onboarding-guide');
                if (guide) guide.style.display = PORTFOLIO.length === 0 ? 'flex' : 'none';
                
                const qsBanner = document.getElementById('quick-scan-banner');
                if (qsBanner) qsBanner.style.display = PORTFOLIO.length === 0 ? 'block' : 'none';
                ...
                if (sumDiv) {
                    ...
                    if (PORTFOLIO.length === 0) { sumDiv.style.display = 'none'; }
                        ...
                        const mkStat = (label, val, sub, color) => `
                            <div style="background: rgba(255,255,255,0.03); padding: 12px 16px; border: 1px solid var(--bg-border); border-radius: 8px; display: flex; flex-direction: row; justify-content: space-between; align-items: center;">
                                <div style="text-align: left;">
                                    <div style="font-size:10px;color:var(--text-3);text-transform:uppercase;letter-spacing:.04em;margin-bottom:4px; font-weight: 600;">${label}</div>
                                    <div style="font-size:10px;color:var(--text-2);">${sub}</div>
                                </div>
                                <div style="text-align: right;">
                                    <div style="font-family:var(--mono);font-size:18px;font-weight:700;color:${color}">${val}</div>
                                </div>
                            </div>`;
                        
                        sumDiv.style.display = 'grid';
                        sumDiv.style.gridTemplateColumns = '1fr';
                        sumDiv.style.gap = '8px';
...
rồi nhấn <strong style="color:var(--gold)">＋ Thêm Vào Danh Mục</strong> để bắt đầu chẩn đoán.
                ...
                console.error('Lỗi trong hàm renderAssetList:', e);
...
<div id="toast-notify"></div>
        ...
        function assetVerdict(a, c) {
            ...
            const isCapGain = a.goal === 'tang-gia';

            const hasCapGain = a.market > a.cost; // đang lời vốn (thị trường > giá mua)

            const cashflowBad = c.cashflow < -20;

            const graceOver = (c.graceRemaining || 0) <= 3; // ân hạn gần/đã hết

            // ─── NÊN BÁN: chỉ khi health cực thấp HOẶC cashflow âm nặng + hết ân hạn

            //     MÀ tài sản KHÔNG phải đang lời vốn với mục tiêu tăng giá ───────────

            if (c.health < 38 || (cashflowBad && graceOver && !(isCapGain && hasCapGain)))

                return { label: '🚨 NÊN BÁN', cls: 'badge-danger' };

            // ─── Mục tiêu tăng giá, đang lời vốn tốt (>15%) nhưng kho trống ────────

            if (isCapGain && hasCapGain && cashflowBad) {

                const gainPct = a.cost > 0 ? (a.market - a.cost) / a.cost * 100 : 0;

                if (gainPct >= 15)

                    return { label: '🚀 LÃI VỐN — CHỐT ĐIỂM?', cls: 'badge-gold' };

                return { label: '⚠️ KHO TRỐNG — ĐANG NUÔI', cls: 'badge-warn' };
            ...
            if (c.roeAnnual < 15 && a.goal === 'cho-thue')

                return { label: '⚠️ TÁI CƠ CẤU (ROE THẤP)', cls: 'badge-warn' };

            if (a.market < c.floorPrice)

                return { label: '🚨 LỖ THỰC TẾ', cls: 'badge-danger' };
            ...
            // ─── TÁI CƠ CẤU LÃI: đang vay lãi thả nổi ≥ 10%, ưu đãi đã hết ──────

            const effectiveFloatRate = a.floatrate || a.rate || 0;

            if (effectiveFloatRate >= 10 && (c.prefRemaining || 0) <= 0 && (c.autoDebt || 0) > 0)

                return { label: '🏦 TÁI CƠ CẤU LÃI', cls: 'badge-warn' };

            // ─── CHO THUÊ NGAY: tài sản trống, đang trả lãi, chưa khai thác ─────────

            const isVacant = !a.rent || a.rent === 0;

            if (isVacant && (c.autoDebt || 0) > 0 && cashflowBad)

                return { label: '📦 CHO THUÊ NGAY', cls: 'badge-warn' };
        ...
        function renderRadarChart(portfolio, calcs) {
            ...
            // ─│ Cải tiến: Tặt cả dùng tỷ lệ, không dùng số tuyệt đối │─

            // Trục 2: Dòng Tiền — Composite: 50% DSCR + 50% Yield

            const _yieldPct = _totalMkt1 > 0 ? (totalCF * 12) / (_totalMkt1 * 1000) * 100 : 0;

            // avgDSCR: tài sản không vay (DSCR null) tính là 2.0 (đã đủ điều kiện)

            const avgDSCR = calcs.reduce((s, { c }) => s + (c.dscr !== null ? Math.min(c.dscr, 3) : 2.0), 0) / n;

            const _dscrScore = Math.max(0, Math.min(1, avgDSCR / 1.5));

            const _yieldScore = Math.max(0, Math.min(1, (_yieldPct + 3) / 9));

            const _cfComposite = 0.5 * _dscrScore + 0.5 * _yieldScore;

            // Trục 4: Tăng Vốn — lãi vốn năm hóa theo benchmark từng pha

            // Ph1=25%/năm, Ph2=15%/năm, Ph3=8%/năm (cần đạt để được điểm 1.0)

            // Trục 4: Tăng Vốn — Composite: 80% Lịch sử + 20% Tiềm năng YoY

            const avgYoY = calcs.reduce((s, { c }) => s + (c.yoy || 0), 0) / n;

            const _gainHist = calcs.reduce((s, { c }) => {

                const annGain = (c.years || 0) > 0 ? (c.gainPct || 0) / c.years : (c.yoy || 0); // Fallback to YoY for new assets

                const bench = c.phase === 1 ? 25 : c.phase === 2 ? 15 : c.phase === 3 ? 8 : 6; // Pha 4 = 6

                return s + Math.max(0, Math.min(1, annGain / bench));

            }, 0) / n;

            const _gainNormVal = Math.min(1, _gainHist * 0.5 + Math.max(0, avgYoY / 30) * 0.5);

            // Trục 5: Đa Dạng — Composite: 60% Pha + 40% Loại Tài Sản

            const _H = [1, 2, 3, 4].reduce((s, p) => {

                const frac = calcs.filter(({ c }) => c.phase === p).length / n;
            ...
            const _uniqueTypes = new Set(portfolio.map(a => a.type)).size;

            const _typeScore = Math.min(1, _uniqueTypes / 3);

            const _divVal = 0.6 * _phaScore + 0.4 * _typeScore;

            const scores = [

                { label: 'Sức Khỏe', val: Math.max(0, Math.min(1, avgHealth / 100)), color: '#10B981', desc: `TB ${avgHealth.toFixed(0)}/100` },

                { label: 'Dòng Tiền', val: _cfComposite, color: '#3B82F6', desc: `DSCR ${avgDSCR.toFixed(2)} | Yield ${_yieldPct >= 0 ? '+' : ''}${_yieldPct.toFixed(1)}%` },

                { label: 'An Toàn Nợ', val: Math.max(0, Math.min(1, 1 - avgLoan / 100)), color: '#EAB308', desc: `Vay TB ${avgLoan.toFixed(0)}%` },

                { label: 'Tăng Vốn', val: _gainNormVal, color: '#F97316', desc: `Lịch sử ${avgGainPct.toFixed(1)}% | Tương lai ${avgYoY.toFixed(1)}%` },

                { label: 'Đa Dạng', val: _divVal, color: '#A855F7', desc: `${(1 / (_H || 1)).toFixed(1)} pha | ${_uniqueTypes} loại TS` },
...
<text x="${CX}" y="${CY + 12}" text-anchor="middle" fill="#9CA3AF" font-size="9" font-family="monospace">/100 Tổng Điểm</text>
            ...
            // Ngưỡng ABCD nâng cao — tiêu chuẩn khắt hơn cho UHNWI consulting

            const grade = overallScore >= 0.75 ? { g: 'A', c: '#10B981', t: 'Danh mục MẠNH' } : overallScore >= 0.55 ? { g: 'B', c: '#EAB308', t: 'Cần Tối Ưu' } : overallScore >= 0.40 ? { g: 'C', c: '#F97316', t: 'Nguy Cơ Trung Bình' } : { g: 'D', c: '#EF4444', t: 'NGUY HIỂM — Cần Cấp Cứu' };
...
<span style="font-size:10px;font-family:monospace;color:${cfColor}">${cfSign}${c.cashflow.toFixed(0)} Tr/th &nbsp;|&nbsp; H:${c.health}</span>
        ...
        function runComboSim() {
            ...
            const newAsset = {
                ...
                cost: (n('cb-cost') + n('cb-capex')) || 0,

                market: n('cb-market') || (n('cb-cost') + n('cb-capex')),

                goal: 'cho-thue',

                loanpct: n('cb-loanpct'),

                debt: n('cb-debt') || (n('cb-cost') * (n('cb-loanpct') || 0) / 100),
            ...
            // Tổng Vốn Đã Đổ (Total Capital Deployed) & Cash Gap Calculation

            const soldAssets = beforeCalcs.filter(({ a }) => COMBO_SELL_IDS.has(a._id));

            const totalSoldMarket = soldAssets.reduce((s, { a }) => s + (a.market || 0), 0);

            const totalSoldDebt = soldAssets.reduce((s, { c }) => s + (c.autoDebt || 0) / 1000, 0);

            // Thuế phí thông minh & Phạt trả nợ theo vòng đời (Hold Years)
            ...
            soldAssets.forEach(({ a, c }) => {

                taxAndFees += (a.market || 0) * 0.03; // Thuế 2% + Phí MG 1%

                const holdYears = currentYear - (a.year || currentYear);

                let penRate = 0;

                if (holdYears <= 1) penRate = 0.03;

                else if (holdYears <= 2) penRate = 0.02;

                else if (holdYears <= 3) penRate = 0.01;

                else penRate = 0; // Miễn phạt nếu >3 năm

                const debtRemaining = (c.autoDebt || 0) / 1000;

                penaltyFees += debtRemaining * penRate;

            });

            const totalFriction = taxAndFees + penaltyFees;

            const cashRecovered = totalSoldMarket - totalSoldDebt - totalFriction;

            // Mua mới

            const newCost = n('cb-cost') || 0;

            const newCapex = n('cb-capex') || 0;

            const newDebtInput = newAsset.debt;

            const cashNeeded = (newCost + newCapex) - newDebtInput;

            const cashGap = cashRecovered - cashNeeded;
            ...
            function metrics(calcs, port, cg = 0) {

                const n = calcs.length;

                // Domino Re-investment: Dòng tiền dư dả (cg > 0) đem gửi tiết kiệm 6%/năm

                const passiveCF = cg > 0 ? (cg * 1000 * 0.06) / 12 : 0;

                return {

                    n,

                    mkt: port.reduce((s, a) => s + (a.market || 0), 0) + Math.max(0, cg), // Thanh khoản dư coi như market hóa tiền mặt

                    debt: port.reduce((s, a) => s + (a.debt || 0), 0),

                    cf: calcs.reduce((s, { c }) => s + c.cashflow, 0) + passiveCF,

                    health: calcs.length > 0 ? Math.round(calcs.reduce((s, { c }) => s + c.health, 0) / n) : 100,

                    equity: calcs.reduce((s, { c }) => s + c.equity, 0) + Math.max(0, cg),
            ...
            const B = metrics(beforeCalcs, beforePortfolio, 0);

            const A = metrics(afterCalcs, afterPortfolio, cashGap);
            ...
            const rows = [
...

                {
                    ...
                    label: 'CF / Tháng', b: (B.cf >= 0 ? '+' : '') + B.cf.toFixed(0) + ' Tr',

                    a: (A.cf >= 0 ? '+' : '') + A.cf.toFixed(0) + ' Tr' + (A.passiveCF > 0 ? `<br><small style="color:var(--emerald)">(${A.passiveCF.toFixed(1)} Tr lãi gộp)</small>` : ''),
...

                {
                    ...
                    label: 'DNA Nhà Đầu Tư', b: PROFILES[B.profile].icon + ' ' + PROFILES[B.profile].name,
            ...
            const summary = [
                ...
                soldNames.length ? `🔴 Bán: ${soldNames.join(', ')}` : null,

                newAsset.cost > 0 ? `🟢 Mua: ${newAsset.name} (Gốc+CapEx: ${newAsset.cost}T)` : null

            ].filter(Boolean).join('&nbsp;&nbsp;|&nbsp;&nbsp;');

            // Báo cáo dòng tiền đối ứng

            let cashGapReport = '';

            if (COMBO_SELL_IDS.size > 0 || newAsset.cost > 0) {

                const cgColor = cashGap >= 0 ? '#10B981' : '#EF4444';

                cashGapReport = `<div style="margin:14px 0;padding:12px 16px;background:rgba(255,255,255,.02);border:1px dashed rgba(255,255,255,.1);border-radius:8px">

<div style="font-size:12px;font-weight:600;margin-bottom:8px;color:#A855F7">⚖️ Cân Đối Dòng Vốn Thực Tế</div>

<div style="display:flex;justify-content:space-between;flex-wrap:wrap;gap:12px;font-size:11px;font-family:monospace;color:var(--text-2)">

<div style="flex:1;min-width:140px">

  <div>Thu hồi Equity: <span style="color:#10B981">+${(totalSoldMarket - totalSoldDebt).toFixed(2)} Tỷ</span></div>

  <div style="color:#EF4444" title="Thuế phí MG 3% + Phạt trả nợ trước hạn 1.5%">Phí ma sát/phạt: -${totalFriction.toFixed(2)} Tỷ</div>

  <div style="font-weight:700;color:var(--text-1);margin-top:4px;border-top:1px solid rgba(255,255,255,.1);padding-top:4px">Tiền ròng thu về: ${(cashRecovered).toFixed(2)} Tỷ</div>

</div>

<div style="font-size:24px;color:rgba(255,255,255,.1);display:flex;align-items:center">-</div>

<div style="flex:1;min-width:140px">

  <div>Mua mới + CapEx: <span style="color:#EF4444">${(newCost + newCapex).toFixed(2)} Tỷ</span></div>

  <div style="color:#10B981">Vay bank đối ứng: +${newDebtInput.toFixed(2)} Tỷ</div>

  <div style="font-weight:700;color:var(--text-1);margin-top:4px;border-top:1px solid rgba(255,255,255,.1);padding-top:4px">Tiền tự có phải chi: ${(cashNeeded).toFixed(2)} Tỷ</div>

</div>

</div>

<div style="margin-top:10px;padding-top:10px;border-top:1px dashed rgba(255,255,255,.1);display:flex;justify-content:space-between;align-items:center">

<div style="font-size:12px">Số dư tiền mặt sau phương án:</div>

<div style="font-size:16px;font-weight:700;font-family:monospace;color:${cgColor}">${cashGap >= 0 ? '+' : ''}${cashGap.toFixed(2)} Tỷ</div>

</div>

<div style="font-size:10px;color:var(--text-3);margin-top:4px;text-align:right">${cashGap < 0 ? '⚠️ Khách hàng cần chuẩn bị thêm tiền mặt.' : '✅ Sinh dư tiền mặt thanh khoản.'}</div>
...
<div style="font-size:11px;color:var(--text-2);margin-bottom:8px;padding:8px 12px;background:rgba(255,255,255,.03);border-radius:6px">${summary}</div>
        ...
        function buildPrescription() {
            ...
            try {
                ...
                calcs.forEach(({ a, c }) => {
                    ...
                    if ((c.graceRemaining || 0) > 0 && (c.graceRemaining || 0) <= 3)

                        alerts.push({ type: 'warn', msg: `${a.name}: Ân hạn còn ${c.graceRemaining} tháng — chuẩn bị tăng dòng tiền ra` });

                    if ((c.prefRemaining || 0) > 0 && (c.prefRemaining || 0) <= 3)

                        alerts.push({ type: 'warn', msg: `${a.name}: Ưu đãi lãi suất hết trong ${c.prefRemaining} tháng` });
                ...
                // Recommendations — chi tiết hơn với số liệu cụ thể

                const matureAssets = calcs.filter(({ a, c }) => !a.isBuyNew && c.years > 0);

                const sorted = [...matureAssets].sort((a, b) => a.c.cashflow - b.c.cashflow);

                const recos = [];

                // R1: Cắt hoại tử — kèm số tiết kiệm theo năm

                if (sorted[0] && sorted[0].c.cashflow < -10) {

                    const lossMo = Math.abs(sorted[0].c.cashflow);

                    const lossYr = (lossMo * 12 / 1000).toFixed(2);

                    recos.push(`🔴 CẮT HOẠI TỬ: "${sorted[0].a.name}" đang đốt ${lossMo.toFixed(0)} Tr/tháng (${lossYr} Tỷ/năm). Nếu thoát trong vòng 3 tháng, bạn tái đầu tư số tiền này vào tài sản Pha 3 sinh lời ngay.`);

                }

                // R2: Dòng tiền âm tổng danh mục — gợi ý refi cụ thể

                if (totalCF < 0) {

                    // Tìm tài sản có lãi vay cao nhất

                    const maxRateA = [...matureAssets].sort((x, y) => (y.a.floatrate || y.a.rate || 0) - (x.a.floatrate || x.a.rate || 0))[0];

                    if (maxRateA) {

                        const curRate = maxRateA.a.floatrate || maxRateA.a.rate || 0;

                        const debtM = maxRateA.c.autoDebt || 0;

                        const savMo = debtM * ((curRate - 8) / 100 / 12);

                        if (curRate > 8 && savMo > 1) {

                            recos.push(`🏦 TÁI CƠ CẤU LÃI: "${maxRateA.a.name}" đang vay ${curRate}%/năm — nếu đàm phán hoặc chuyển ngân hàng về 8%/năm, tiết kiệm ~${savMo.toFixed(0)} Tr/tháng = ${(savMo * 12 / 1000).toFixed(2)} Tỷ/năm.`);
                ...
                if (!calcs.find(({ c }) => c.phase === 3))
                    ...
                    recos.push('🩸 BƠM MÁU: Chưa có tài sản Pha 3 dòng tiền. Ưu tiên mua 1 BĐS cho thuê dòng tiền dương trong 6 tháng tới để cân bằng áp lực lãi vay.');

                // R4: Surgery Best Scenario linkage

                const lastSurgery = window._LAST_SCENARIOS_RESULTS;

                if (lastSurgery && lastSurgery.length > 0) {

                    const topSc = lastSurgery.reduce((best, cur) => {

                        const bRow = best.rows.find(r => r.label === 'DSCR');

                        const cRow = cur.rows.find(r => r.label === 'DSCR');

                        return (cRow?.rawVal || 0) > (bRow?.rawVal || 0) ? cur : best;

                    }, lastSurgery[0]);

                    const topDscr = topSc.rows.find(r => r.label === 'DSCR');

                    const topCf = topSc.rows.find(r => r.label === 'Cashflow Ròng / Tháng');

                    if (topDscr && topDscr.rawVal >= 1) {

                        recos.push(`🏆 CHIẾN LƯỢC TỐI ƯU (từ Surgery Simulator): Kịch bản "${topSc.sc.title}" đưa DSCR lên ${(topDscr.rawVal).toFixed(2)} với CF ${topCf ? (topCf.rawVal >= 0 ? '+' : '') + topCf.rawVal.toFixed(0) + ' Tr/tháng' : ''}. Đây là phương án được đề nghị thực thi ngay.`);
                ...
                const mergedRowsHTML = calcs.map(({ a, c }, i) => {
                    ...
                    const cfColor = c.cashflow >= 0 ? '#059669' : '#DC2626';

                    const roeColor = c.roeAnnual >= 15 ? '#059669' : c.roeAnnual >= 8 ? '#D97706' : '#DC2626';

                    const floorColor = a.market >= (c.floorPrice || 0) ? '#059669' : '#DC2626';

                    const hColor = c.health >= 75 ? '#059669' : c.health >= 50 ? '#D97706' : '#DC2626';

                    const PHASE_NAMES = ['', 'Pha 1', 'Pha 2', 'Pha 3', 'Pha 4'];

                    const PHASE_COLORS = ['', '#3B82F6', '#EAB308', '#10B981', '#9CA3AF'];
                    ...
                    if ((c.graceRemaining || 0) > 0) noteArr.push('💣 ân hạn ' + c.graceRemaining + 'T');

                    if ((c.prefRemaining || 0) > 0 && (c.prefRemaining || 0) <= 3) noteArr.push('⚡ ƯD còn ' + c.prefRemaining + 'T');

                    return '<tr style="border-bottom:1px solid #F1F5F9;' + rowBg + '">'

                        + '<td style="padding:7px 8px;font-weight:600;color:#1C1C2E">' + a.name + '</td>'

                        + '<td style="padding:7px 8px;text-align:center;font-family:monospace;font-size:10px;color:#374151">' + a.cost.toFixed(1) + 'T / <span style="color:#D97706">' + a.market.toFixed(1) + 'T</span></td>'

                        + '<td style="padding:7px 8px;text-align:center;font-family:monospace;font-weight:700;color:' + cfColor + '">' + (c.cashflow >= 0 ? '+' : '') + c.cashflow.toFixed(0) + ' Tr</td>'

                        + '<td style="padding:7px 8px;text-align:center;font-family:monospace;font-weight:700;color:' + dscrColor + '">' + dscrTxt + '</td>'

                        + '<td style="padding:7px 8px;text-align:center;font-family:monospace;color:' + roeColor + '">' + c.roeAnnual.toFixed(1) + '% / ' + (c.cagr > -999 ? c.cagr.toFixed(1) : '?') + '%</td>'

                        + '<td style="padding:7px 8px;text-align:center;font-family:monospace;color:' + floorColor + '">' + (c.floorPrice || 0).toFixed(2) + 'T</td>'

                        + '<td style="padding:7px 8px;text-align:center;font-family:monospace;font-weight:700;color:' + hColor + '">' + c.health + '</td>'

                        + '<td style="padding:7px 8px;text-align:center;font-size:10px;font-weight:700;color:' + (PHASE_COLORS[c.phase] || '#9CA3AF') + '">' + (PHASE_NAMES[c.phase] || '?') + '</td>'

                        + '<td style="padding:7px 8px;font-size:9px;color:#374151">' + (noteArr.join(' | ') || '\u2014') + '</td>'
                ...
                const _avgHr = _totalMktW > 0

                    ? calcs.reduce((s, { a, c }) => s + (a.market || 0) * c.health, 0) / _totalMktW

                    : calcs.reduce((s, { c }) => s + c.health, 0) / calcs.length;
                ...
                // Đồng bộ logic tính điểm chính xác bằng "Tỷ lệ chuẩn hóa" từ renderRadarChart

                const _v1r = Math.max(0, Math.min(1, _avgHr / 100)); // Sức Khỏe

                const _totalMkt1 = calcs.reduce((s, { a }) => s + (a.market || 0), 0);

                const _yieldPct = _totalMkt1 > 0 ? (_cfr * 12) / (_totalMkt1 * 1000) * 100 : 0;

                const _avgDSCR = calcs.reduce((s, { c }) => s + (c.dscr !== null ? Math.min(c.dscr, 3) : 2.0), 0) / calcs.length;

                const _dscrScore = Math.max(0, Math.min(1, _avgDSCR / 1.5));

                const _yieldScore = Math.max(0, Math.min(1, (_yieldPct + 3) / 9));

                const _v2r = 0.5 * _dscrScore + 0.5 * _yieldScore; // Dòng Tiền Composite: 50% DSCR + 50% Yield

                const _v3r = Math.max(0, Math.min(1, 1 - _avgLr / 100)); // An Toàn Nợ

                // Tăng Vốn: 50% Lịch sử + 50% Tiềm năng (YoY) — đồng bộ với Radar
                ...
                // Điểm Cắt Lựa Chọn ABCD Chuẩn như Radar
                ...
                if (window._IS_VIRTUAL_MODE && window.SESSION_PORTFOLIO) {

                    const curCalcs = window.SESSION_PORTFOLIO.map(a => ({ a, c: calcAsset(a) }));

                    const curCF = curCalcs.reduce((s, { c }) => s + c.cashflow, 0);

                    const curMkt = window.SESSION_PORTFOLIO.reduce((s, a) => s + (a.market || 0), 0);

                    const curDebt = curCalcs.reduce((s, { c }) => s + (c.autoDebt || 0) / 1000, 0);

                    const dtCF = totalCF - curCF;

                    const dtMkt = totalMkt - curMkt;

                    const dtDebt = totalDebt - curDebt;

                    deltaHtml = `<div style="margin-top:14px;padding:12px 16px;background:#ECFDF5;border:1px solid #A7F3D0;border-radius:8px;font-size:12px;color:#065F46;display:flex;justify-content:space-between;align-items:center;">

            <div style="font-weight:700">🔀 SO VỚI HIỆN TẠI (DELTA):</div>

            <div>NAV: <strong style="color:${dtMkt >= 0 ? '#059669' : '#DC2626'}">${dtMkt >= 0 ? '+' : ''}${dtMkt.toFixed(1)} Tỷ</strong></div>

            <div>Dòng Tiền: <strong style="color:${dtCF >= 0 ? '#059669' : '#DC2626'}">${dtCF >= 0 ? '+' : ''}${dtCF.toFixed(1)} Tr/th</strong></div>

            <div>Dư Nợ: <strong style="color:${dtDebt <= 0 ? '#059669' : '#DC2626'}">${dtDebt > 0 ? '+' : ''}${dtDebt.toFixed(1)} Tỷ</strong></div>
...
<div style="display:flex;flex-direction:column;align-items:flex-end;gap:6px">
  ...
  <!-- Badge mode indicator — click để đổi, luôn in được -->

  <div onclick="toggleVirtualMode()" style="background:${window._IS_VIRTUAL_MODE ? '#059669' : '#374151'};color:#fff;border:1px solid ${window._IS_VIRTUAL_MODE ? '#10b981' : '#4B5563'};border-radius:6px;padding:6px 12px;font-size:11px;font-weight:700;letter-spacing:.03em;cursor:pointer;transition:opacity .2s" onmouseover="this.style.opacity='.8'" onmouseout="this.style.opacity='1'">

    ${window._IS_VIRTUAL_MODE ? '✅ DANH MỤC SAU CƠ CẤU' : '📊 DANH MỤC HIỆN TẠI'}
...
<div style="font-size:13px;font-weight:700;color:#1C1C2E;text-transform:uppercase;letter-spacing:.06em;margin-bottom:14px">II. DNA NHÀ ĐẦU TƯ</div>
...
<div style="font-size:13px;font-weight:700;color:#1C1C2E;text-transform:uppercase;letter-spacing:.06em;margin-bottom:14px">III. DANH MỤC TÀI SẢN — PHÂN TÍCH TÀI CHÍNH CHI TIẾT</div>

<div style="overflow-x:auto">

<table style="width:100%;border-collapse:collapse;font-size:11px">

<thead><tr style="background:#EEF2FF;border-bottom:2px solid #C7D2FE">

  <th style="text-align:left;padding:8px">Tài Sản</th>

  <th style="text-align:center;padding:8px">Giá Vốn / HT</th>

  <th style="text-align:center;padding:8px">CF/Th</th>

  <th style="text-align:center;padding:8px">DSCR</th>

  <th style="text-align:center;padding:8px">ROE / CAGR</th>

  <th style="text-align:center;padding:8px">Sàn HV</th>

  <th style="text-align:center;padding:8px">Health</th>

  <th style="text-align:center;padding:8px">Pha</th>

  <th style="text-align:left;padding:8px">Lưu ý</th>
...
* CF = Dòng tiền thuần/tháng (Tr) &nbsp;|&nbsp; DSCR = CF / Trả Nợ &nbsp;|&nbsp; ROE/CAGR = Lợi Nhuận / Vốn &nbsp;|&nbsp; Sàn HV = Giá Hòa Vốn
...
${recos.map((r, i) => `<div style="padding:10px 14px;margin-bottom:8px;background:#fff;border-left:4px solid ${i === 0 ? '#DC2626' : '#10B981'};border-radius:0 6px 6px 0;font-size:12px;color:#1C1C2E">

<strong style="color:${i === 0 ? '#DC2626' : '#15803D'}">${i + 1}.</strong> ${r}
...
`;
                ...
                // Removed incorrect wrapper overwrite hack

                if (empty) empty.style.display = 'none';

                if (printBtn) printBtn.style.display = 'inline-flex';

                const hint = document.getElementById('rx-print-hint');

                if (hint) hint.style.display = 'inline';

            } catch (e) {

                console.error('Lỗi trong hàm buildPrescription:', e);
        ...
        function calcAsset(a) {
            ...
            // TÍNH TOÁN CAGR (Tỷ suất sinh lời kép hằng năm)

            let cagr = 0;

            if (equity > 0 && roeTotal > -100) {

                cagr = (Math.pow(1 + roeTotal / 100, 1 / holdYears) - 1) * 100;

            } else if (roeTotal <= -100) {
            ...
            // Trụ 2: ROE (tấn công — sinh lời trên vốn tự có, chuẩn 15%/năm cho VIP)

            const roeScore = Math.max(0, Math.min(roeAnnual / 15 * 100, 100));
            ...
            // Hệ số mới: DSCR 35% (phòng thủ), ROE 30% (tấn công), Lãi Vốn 25% (tăng trưởng), Market 10%

            // Ph1/Ph2 không cho thuê → không bị phạt ROE quá nặng so với phiên bản cũ

            const health = Math.round(dscrScore * 0.35 + roeScore * 0.30 + gainScore * 0.25 + marketScore * 0.10);

            const unitPrice = (a.area || 0) > 0 ? (a.market * 1000) / a.area : 0;

            let alerts = [];

            const isHoneymoon = a.isBuyNew || years === 0;

            if (isHoneymoon) {

                alerts.push(`🌱 Tài sản mới cơ cấu — Cần thời gian 1-2 năm để hấp thụ chi phí chìm (thuế/phí) và bắt đầu chu kỳ đánh giá.`);

            } else {

                if (roeAnnual < 15 && a.goal === 'cho-thue') {

                    alerts.push(`📉 ROE thực = ${roeAnnual.toFixed(1)}% — Dưới chuẩn 15%. Gửi tiết kiệm còn có lợi hơn.`);

                }

                if (a.market < floorPrice) {

                    alerts.push(`🛈 Giá TT ${a.market}Tỷ thấp hơn Giá sàn hòa vốn ${floorPrice.toFixed(2)}Tỷ. Bán hôm nay = lỗ thực.`);
            ...
            return {
                ...
                floorPrice, phase, phaseWarning, alerts, deliveryNote, cycle, yoy, viewsTin, cat_lo, health, distData, intOnly, principal, years, unitPrice, autoDebt: debtB, cagr, graceRemaining, prefRemaining
...
<div class="card" style="border-color:${pCfg.color}44;background:rgba(0,0,0,0.2);height:100%;box-sizing:border-box;display:flex;flex-direction:column">

<div class="card-header">

<span class="card-title">🧬 DNA Nhà Đầu Tư</span>

</div>

<!-- Icon + Tên loại: trung tâm -->

<div style="text-align:center;padding:20px 16px 16px">

<div style="font-size:52px;line-height:1;filter:drop-shadow(0 0 12px ${pCfg.color}66)">${pCfg.icon}</div>

<div style="font-family:var(--mono);font-size:13px;color:${pCfg.color};font-weight:700;letter-spacing:.08em;margin-top:10px">${pCfg.name}</div>

<div style="font-size:11px;color:var(--text-3);margin-top:3px;letter-spacing:.04em">${pCfg.eng}</div>

</div>

<!-- Đường kẻ phân cách -->

<div style="height:1px;background:${pCfg.color}22;margin:0 16px"></div>

<!-- Nội dung phân tích: trải đều phần còn lại -->

<div style="flex:1;display:flex;flex-direction:column;justify-content:space-evenly;padding:16px">

<div style="font-size:12px;color:var(--text-1);line-height:1.65;padding:10px 12px;background:rgba(255,255,255,.03);border-radius:8px;border-left:3px solid ${pCfg.color}66">

${pCfg.desc}

</div>

<div style="font-size:12px;color:var(--yellow);padding:8px 12px;background:rgba(245,158,11,.05);border-radius:8px">

⚠️ <strong>Riủi ro cốt lõi:</strong> ${pCfg.weakness}

</div>

<div style="font-size:12px;color:${pCfg.color};padding:8px 12px;background:${pCfg.color}0D;border-radius:8px">

💊 <strong>Tỏa thuốc:</strong> ${pCfg.rx}
...
`;
                ...
                // Tính toán tổng danh mục (dùng các biến đã có: totalMarket, totalCF, avgHealth)

                const totalDebt = calcs.reduce((s, { c }) => s + (c.autoDebt > 0 ? c.autoDebt / 1000 : 0), 0);

                // totalMarket, totalCF, avgHealth đã được khai báo ở trên (dùng lại)
                ...
                const graceCluster = calcs.filter(({ c }) => (c.graceRemaining || 0) > 0 && (c.graceRemaining || 0) <= 3);
                ...
                (() => {
                    ...
                    const sortedByCF = [...calcs].sort((a, b) => a.c.cashflow - b.c.cashflow);

                    const sortedByH = [...calcs].sort((a, b) => b.c.health - a.c.health); // Descending health

                    const worstCFAsset = sortedByCF[0];

                    const bestAsset = sortedByH[0];

                    const avgLoanGlobal = n > 0 ? (calcs.reduce((s, { a }) => s + (a.loanpct || 0), 0) / n) : 0;

                    const totalDebtVal = calcs.reduce((s, { c }) => s + c.totalDebt, 0);

                    const totalMarketVal = calcs.reduce((s, { a }) => s + (a.market || 0), 0);

                    const portfYield = totalMarketVal > 0 ? (totalCF * 12) / (totalMarketVal * 1000) * 100 : 0;

                    const ph1Count = calcs.filter(({ c }) => c.phase === 1).length;

                    const ph2Count = calcs.filter(({ c }) => c.phase === 2).length;

                    const ph3Count = calcs.filter(({ c }) => c.phase === 3).length;

                    const ph2Value = calcs.filter(({ c }) => c.phase === 2).reduce((s, { a }) => s + (a.market || 0), 0);

                    const ph2ValuePct = totalMarketVal > 0 ? (ph2Value / totalMarketVal) * 100 : 0;

                    const recos = [];

                    // 1. CẮT HOẠI TỬ: Chết lâm sàng (DSCR < 0.3) hoặc Âm nặng (>50Tr/th) thay vì fix 10Tr

                    if (worstCFAsset && (worstCFAsset.c.dscr !== null && worstCFAsset.c.dscr < 0.3 || worstCFAsset.c.cashflow < -50)) {

                        recos.push(`🔴 <strong>Cắt hoại tử (Ưu tiên Cao):</strong> "${worstCFAsset.a.name}". Dòng tiền thu không bù nổi lãi (DSCR ${worstCFAsset.c.dscr !== null ? worstCFAsset.c.dscr.toFixed(2) : 'N/A'}). Giải phóng ${worstCFAsset.a.market} Tỷ để cứu toàn hệ thống trong 0–3 tháng tới.`);

                    }

                    // 2. HẠ ĐÒN BẨY: Nợ toàn cục > 50% & Yield âm

                    if (avgLoanGlobal > 50 && portfYield < 0) {

                        recos.push(`⚠️ <strong>Hạ đòn bẩy:</strong> Danh mục vay TB ${avgLoanGlobal.toFixed(0)}% và Yield tổng đang âm (${portfYield.toFixed(1)}%). Phải bán bớt 1 tài sản yếu nhất để dội vào gốc, giảm nợ về ngưỡng an toàn <40%.`);

                    }

                    // 3. TÁI CƠ CẤU PHA 2: Chôn vốn nhiều ở Pha 2 nhưng thiếu dòng tiền Pha 3

                    if (ph2ValuePct > 40 && ph3Count === 0) {

                        recos.push(`🔄 <strong>Phục sinh Pha 2:</strong> >${ph2ValuePct.toFixed(0)}% vốn hóa đang nằm ở Pha 2 chờ tăng trưởng nhưng danh mục khát dòng tiền. Hãy cân nhắc <em>Xây dựng / Khai thác cho thuê</em> trên vùng này để tự tạo Pha 3 thay vì mua mới.`);

                    }

                    // 4. BƠM MÁU: Thiếu Pha 3 hoàn toàn (và không thể cải tạo Pha 2)

                    if (ph3Count === 0 && ph2ValuePct <= 40) {

                        recos.push(`⚡ <strong>Bơm máu mạch chủ:</strong> Mất cân đối nghiêm trọng, danh mục thiếu màng lọc phòng thủ Pha 3. Ưu tiên mua 1 BDS Tạo dòng tiền (Chung cư/Nhà dòng tiền) để gánh lãi.`);

                    }

                    // 5. MỞ RỘNG: Thiếu Pha 1 tăng trưởng cho portfolio đã phòng thủ khỏe

                    if (ph1Count === 0 && ph3Count > 0 && profKey !== 'prey' && avgLoanGlobal < 40) {

                        recos.push(`🚀 <strong>Mở rộng bờ cõi:</strong> Dòng tiền đã vững, nợ thấp. Tới lúc dồn 20–30% vốn săn "Hạt giống" Pha 1 (Đất nền rỉ rách theo quy hoạch/hạ tầng lớn) cho kỳ vọng X2-X3 chu kỳ tới.`);

                    }

                    // 6. GIỮ CHẶT (HẠT NGỌC): Luôn có ít nhất 1 tài sản tốt nhất để vinh danh

                    if (bestAsset && bestAsset.c.health >= 70 && bestAsset.c.cashflow >= 0) {

                        recos.push(`🟢 <strong>Giữ chặt ngọc quý:</strong> "${bestAsset.a.name}" (Health: ${bestAsset.c.health}/100, DSCR: ${bestAsset.c.dscr !== null ? bestAsset.c.dscr.toFixed(2) : '>1'}). Tuyệt đối không đem bán tài sản lõi này dù có bị ép giá.`);

                    }

                    if (recos.length === 0)

                        recos.push(`✅ Danh mục đang cân bằng siêu cấp (ROE ổn, Tự nuôi được nợ). Tiếp tục nắm giữ và theo dõi Cycle Index.`);
...
${recos.map(r => `<div style="font-size:12px;color:var(--text-2);padding:5px 0;border-bottom:1px solid rgba(255,255,255,0.04)">${r}</div>`).join('')}
                ...
                // Tổng giá trị thị trường toàn danh mục (dùng để tính % theo giá trị)

                const _totalMktMatrix = calcs.reduce((s, { a }) => s + (a.market || 0), 0);
                ...
                const _matrixValStr = _totalMktMatrix >= 1 ? _totalMktMatrix.toFixed(1) + ' Tỷ' : (_totalMktMatrix * 1000).toFixed(0) + ' Tr';

                document.getElementById('diag-matrix-note').textContent = `${n} tài sản · ${_matrixValStr}`;

                document.getElementById('diag-matrix').innerHTML = PHASE_CFG.map((cfg, pi) => {

                    const items = phData[pi];

                    const phaseVal = items.reduce((s, { a }) => s + (a.market || 0), 0);

                    const pct = _totalMktMatrix > 0 ? Math.round(phaseVal / _totalMktMatrix * 100) : 0;

                    const phaseValStr = phaseVal >= 1 ? phaseVal.toFixed(1) + ' Tỷ' : (phaseVal * 1000).toFixed(0) + ' Tr';
...
<span style="font-family:var(--mono);font-size:12px;color:var(--text-2)">${items.length} TS · <strong style="color:${cfg.color}">${phaseValStr}</strong> (${pct}%)</span>
...
· ${a.name} <span style="color:var(--text-3);font-family:var(--mono)">${a.market ? a.market.toFixed(1) + ' Tỷ' : ''}</span>
...
<span>ROE: <strong style="color:${c.roeAnnual >= 15 ? 'var(--emerald)' : c.roeAnnual >= 8 ? 'var(--yellow)' : 'var(--red)'}">${c.roeAnnual.toFixed(1)}%</strong> <span style="font-size:10px;color:var(--text-3)" title="Sinh lời kép">(CAGR: ${c.cagr.toFixed(1)}%)</span></span>
...
<span>Ân hạn: <strong style="color:${(c.graceRemaining || 0) > 0 && (c.graceRemaining || 0) <= 3 ? 'var(--yellow)' : 'var(--text-1)'}">${c.graceRemaining || 0} th</strong></span>
                ...
                if (md) {
                    ...
                    // Store owned districts globally for renderMktTable

                    window._MKT_OWN = new Set(portfolio.map(a => a.district).filter(Boolean));

                    // Render sort buttons

                    const SORT_BTNS = [

                        { key: 'pha', label: '📋 Vùng', title: 'Sắp xếp theo Vùng (Mặc định)' },

                        { key: 'abc', label: '🔤 A→Z', title: 'Theo thứ tự ABC tiếng Việt' },

                        { key: 'cycle', label: '🌡️ Cycle↓', title: 'Nhiệt thị trường cao nhất trước' },

                        { key: 'heat', label: '🔥 Heat↓', title: 'Chỉ số nóng tổng hợp (Cycle + V/T + YoY + Sức khỏe) — cao nhất trước' },

                        { key: 'yoy', label: '📈 YoY↓', title: 'Tăng giá YoY nhà riêng cao nhất trước (nguồn: batdongsan.com.vn)' },

                        { key: 'potential', label: '💎 Tiềm Năng↓', title: 'Điểm tiềm năng tăng trưởng dài hạn cao nhất trước' },

                    ];

                    window._MKT_SORT = window._MKT_SORT || 'pha';

                    document.getElementById('diag-market-sort').innerHTML = SORT_BTNS.map(b =>

                        `<button title="${b.title}" onclick="renderMktTable('${b.key}')" id="mkt-sort-${b.key}"

              style="font-size:10px;padding:3px 10px;border-radius:20px;border:1px solid;

              cursor:pointer;transition:all 0.15s;

              background:${window._MKT_SORT === b.key ? 'var(--gold)' : 'transparent'};

              color:${window._MKT_SORT === b.key ? '#000' : 'var(--text-2)'};

              border-color:${window._MKT_SORT === b.key ? 'var(--gold)' : 'var(--bg-border)'}">

              ${b.label}</button>`

                    ).join('');

                    renderMktTable(window._MKT_SORT);
        ...
        window.renderMktTable = function (sortKey) {
            window._MKT_SORT = sortKey;
            const md = window.MARKET_DATA;
            if (!md) return;
            ['pha', 'yoy', 'cycle', 'abc', 'heat', 'potential'].forEach(k => {
                const btn = document.getElementById(`mkt-sort-${k}`);
                if (btn) {
                    btn.style.background = k === sortKey ? 'var(--gold)' : 'transparent';
                    btn.style.color = k === sortKey ? '#000' : 'var(--text-2)';
                    btn.style.borderColor = k === sortKey ? 'var(--gold)' : 'var(--bg-border)';
                }
            });
            const PHASE_EXACT = {
                'Đan Phượng': 1, 'Sóc Sơn': 1, 'Mê Linh': 1, 'Thạch Thất': 1, 'Từ Sơn (BN)': 1, 'Văn Giang (HY)': 1,
                'Hoài Đức': 2, 'Đông Anh': 2, 'Hoàng Mai': 2, 'Thanh Trì': 2, 'Gia Lâm': 2,
                'Đống Đa': 3, 'Cầu Giấy': 3, 'Hà Đông': 3, 'Thanh Xuân': 3, 'Hai Bà Trưng': 3,
                'Ba Đình': 3, 'Nam Từ Liêm': 3, 'Tây Hồ': 3, 'Bắc Từ Liêm': 3, 'Hoàn Kiếm': 3, 'Long Biên': 3
            };
            const P_COL = { 1: '#60a5fa', 2: '#fbbf24', 3: '#34d399' };
            const PHA_CUSTOM_ORDER = [
                'Hoàn Kiếm', 'Ba Đình', 'Hai Bà Trưng', 'Đống Đa', 'Tây Hồ',
                'Cầu Giấy', 'Thanh Xuân', 'Long Biên', 'Hoàng Mai', 'Bắc Từ Liêm',
                'Nam Từ Liêm', 'Hà Đông', 'Thanh Trì', 'Hoài Đức', 'Đông Anh',
                'Gia Lâm', 'Sóc Sơn', 'Mê Linh', 'Đan Phượng', 'Thạch Thất',
                'Văn Giang (HY)', 'Từ Sơn (BN)'
            ];
            const PHA_ORDER_MAP = {};
            PHA_CUSTOM_ORDER.forEach((name, i) => PHA_ORDER_MAP[name] = i);
            let arr = [...md.districts];
            const norm = str => str.toLowerCase().replace(/đ/g, 'd').normalize("NFD").replace(/[\u0300-\u036f]/g, "");
            if (sortKey === 'pha') arr.sort((a, b) => (PHA_ORDER_MAP[a.name] ?? 99) - (PHA_ORDER_MAP[b.name] ?? 99));
            else if (sortKey === 'yoy') arr.sort((a, b) => (b.yoy || 0) - (a.yoy || 0));
            else if (sortKey === 'cycle') arr.sort((a, b) => (b.cycle || 0) - (a.cycle || 0));
            else if (sortKey === 'abc') arr.sort((a, b) => norm(a.name).localeCompare(norm(b.name)));
            else if (sortKey === 'heat') arr.sort((a, b) => (b.heat || 0) - (a.heat || 0));
            else if (sortKey === 'potential') arr.sort((a, b) => (b.potential || 0) - (a.potential || 0));

            // ── Summary Cards ────────────────────────────────────────────
            const byHeat = [...md.districts].sort((a, b) => (b.heat || 0) - (a.heat || 0));
            const byYoy = [...md.districts].sort((a, b) => (b.yoy || 0) - (a.yoy || 0));
            const byVt = [...md.districts].filter(d => d.views_tin > 0).sort((a, b) => (b.views_tin || 0) - (a.views_tin || 0));
            const th = byHeat[0], ty = byYoy[0], tv = byVt[0];
            const summaryHTML = `
        <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:10px;margin-bottom:16px">
          <div style="background:rgba(212,175,55,0.08);border:1px solid rgba(212,175,55,0.3);border-radius:10px;padding:12px">
            <div style="font-size:9px;color:var(--text-3);text-transform:uppercase;letter-spacing:.07em;margin-bottom:5px">🔥 Khu Nóng Nhất</div>
            <div style="font-size:14px;font-weight:800;color:var(--gold)">${th ? th.name : '—'}</div>
            <div style="font-size:10px;color:var(--text-2);margin-top:3px">Heat <strong style="color:var(--gold)">${th ? th.heat : '—'}</strong>/100 · Cycle ${th ? th.cycle : '—'}</div>
          </div>
          <div style="background:rgba(0,194,122,0.06);border:1px solid rgba(0,194,122,0.25);border-radius:10px;padding:12px">
            <div style="font-size:9px;color:var(--text-3);text-transform:uppercase;letter-spacing:.07em;margin-bottom:5px">📈 YoY NR Cao Nhất</div>
            <div style="font-size:14px;font-weight:800;color:var(--emerald)">${ty ? ty.name : '—'}</div>
            <div style="font-size:10px;color:var(--text-2);margin-top:3px">NR <strong style="color:var(--emerald)">${ty ? ty.yoy + '%' : '—'}</strong> · CC ${ty ? (ty.yoy_cc || 0) + '%' : '—'}</div>
          </div>
          <div style="background:rgba(99,102,241,0.06);border:1px solid rgba(99,102,241,0.25);border-radius:10px;padding:12px">
            <div style="font-size:9px;color:var(--text-3);text-transform:uppercase;letter-spacing:.07em;margin-bottom:5px">⚡ Thanh Khoản Tốt Nhất</div>
            <div style="font-size:14px;font-weight:800;color:#818cf8">${tv ? tv.name : '—'}</div>
            <div style="font-size:10px;color:var(--text-2);margin-top:3px">V/T <strong style="color:#818cf8">${tv ? tv.views_tin : '—'}</strong> · ${tv ? (tv.views >= 1000 ? (tv.views / 1000).toFixed(1) + 'K' : tv.views) + ' views' : '—'}</div>
          </div>
        </div>`;

            // ── Delta arrow helper ────────────────────────────────────────
            const dArrow = v => v == null
                ? `<span style="color:var(--text-3);font-size:10px">ΔGiá —</span>`
                : v > 0 ? `<span style="color:var(--emerald);font-size:10px">↑${v.toFixed(1)}%</span>`
                    : v < 0 ? `<span style="color:var(--red);font-size:10px">↓${Math.abs(v).toFixed(1)}%</span>`
                        : `<span style="color:var(--text-3);font-size:10px">→0%</span>`;

            // ── Build rows ────────────────────────────────────────────────
            const rows = arr.map(d => {
                const isOwned = window._MKT_OWN && window._MKT_OWN.has(d.name);
                const ph = PHASE_EXACT[d.name] || 1;
                const pc = P_COL[ph] || '#60a5fa';
                const hs = d.heat || 0;
                const hc = hs >= 55 ? 'var(--emerald)' : hs >= 35 ? 'var(--yellow)' : 'var(--red)';
                const cc = d.cycle > 70 ? 'var(--red)' : d.cycle > 50 ? 'var(--yellow)' : 'var(--emerald)';
                const sig = hs >= 55 ? { i: '🔥', l: 'Nóng', c: 'var(--emerald)', b: 'rgba(0,194,122,0.12)' }
                    : hs >= 35 ? { i: '⚡', l: 'Ổn', c: 'var(--yellow)', b: 'rgba(245,158,11,0.12)' }
                        : { i: '❄️', l: 'Lạnh', c: 'var(--red)', b: 'rgba(239,68,68,0.12)' };
                const vtB = d.views_tin >= 12 ? '🔥' : d.views_tin >= 7 ? '⚡' : d.views_tin < 3 ? '❄️' : '';
                const vtC = d.views_tin < 3 ? 'var(--red)' : d.views_tin < 7 ? 'var(--yellow)' : 'var(--emerald)';
                const vtCc = d.views_tin_cc != null ? d.views_tin_cc : null;
                const vtCcB = vtCc == null ? '' : vtCc >= 12 ? '🔥' : vtCc >= 7 ? '⚡' : vtCc < 3 ? '❄️' : '';
                const vtCcC = vtCc == null ? 'var(--text-3)' : vtCc < 3 ? 'var(--red)' : vtCc < 7 ? 'var(--yellow)' : 'var(--emerald)';
                const vs = d.views > 0 ? (d.views >= 1000 ? (d.views / 1000).toFixed(1) + 'K' : d.views) : '—';
                return `<tr style="${isOwned ? 'background:rgba(212,175,55,0.06);border-left:3px solid var(--gold-dim);' : ''}">
<td style="padding:8px 10px">
  <div style="font-size:12px;font-weight:${isOwned ? 700 : 500};color:${isOwned ? 'var(--gold)' : 'var(--text-1)'}">${isOwned ? '📍 ' : ''}${d.name}</div>
  <span style="font-size:9px;padding:1px 5px;border-radius:4px;background:${pc}22;color:${pc};font-weight:600">Pha ${ph}</span>
</td>
<td style="padding:8px 10px;text-align:center">
  <div style="font-size:15px;font-weight:800;color:${hc}">${hs}</div>
  <div style="height:3px;background:rgba(255,255,255,0.08);border-radius:2px;margin-top:3px">
    <div style="height:100%;width:${Math.min(hs, 100)}%;background:${hc};border-radius:2px"></div>
  </div>
</td>
<td style="padding:8px 10px;text-align:center">
  <span style="font-size:12px;font-family:monospace;color:${cc};font-weight:700">${d.cycle}</span>
  <div style="font-size:9px;color:var(--text-3)">/100</div>
</td>
<td style="padding:8px 10px">
  <div style="font-size:12px;font-weight:700;color:var(--text-1)">${d.gia} <span style="font-size:9px;color:var(--text-3);font-weight:400">tr/m²</span></div>
  <div style="margin-top:2px">${dArrow(d.delta_gia)}</div>
  <div style="font-size:10px;margin-top:2px">YoY: <strong style="color:${(d.yoy || 0) >= 20 ? 'var(--emerald)' : 'var(--text-2)'}">${d.yoy}%</strong> &nbsp;CL: <span style="color:${(d.cat_lo || 0) > 20 ? 'var(--red)' : 'var(--text-2)'}">${d.cat_lo}%</span></div>
</td>
<td style="padding:8px 10px">
  <div style="font-size:12px;font-weight:600;color:var(--text-2)">${d.gia_cc != null ? d.gia_cc : '—'} <span style="font-size:9px;color:var(--text-3);font-weight:400">tr/m²</span></div>
  <div style="margin-top:2px">${dArrow(d.delta_gia_cc != null ? d.delta_gia_cc : null)}</div>
  <div style="font-size:10px;margin-top:2px">YoY: <strong style="color:${(d.yoy_cc || 0) >= 20 ? 'var(--emerald)' : (d.yoy_cc || 0) < 0 ? 'var(--red)' : 'var(--text-2)'}">${d.yoy_cc != null ? d.yoy_cc + '%' : '—'}</strong> &nbsp;CL: <span style="color:${(d.cat_lo_cc || 0) > 20 ? 'var(--red)' : 'var(--text-2)'}">${d.cat_lo_cc != null ? d.cat_lo_cc + '%' : '—'}</span></div>
</td>
<td style="padding:8px 10px;text-align:center">
  <div style="font-size:11px;font-weight:700;color:${vtC}">${vtB} ${d.views_tin} <span style="font-size:9px;color:var(--text-3);font-weight:400">NR</span></div>
  <div style="font-size:9px;color:var(--text-3);margin:2px 0">${vs} views · ${d.tin || '—'} tin</div>
  <div style="font-size:11px;font-weight:600;color:${vtCcC}">${vtCc != null ? vtCcB + ' ' + vtCc : '—'} <span style="font-size:9px;color:var(--text-3);font-weight:400">CC</span></div>
</td>
<td style="padding:8px 10px;text-align:center">
  <span style="display:inline-block;padding:4px 10px;border-radius:20px;font-size:11px;font-weight:700;background:${sig.b};color:${sig.c};border:1px solid ${sig.c}44;white-space:nowrap">${sig.i} ${sig.l}</span>
</td>
</tr>`;
            }).join('');

            const tableHTML = `<table class="mkt-table" style="min-width:720px">
<thead><tr>
  <th style="text-align:left">Khu Vực</th>
  <th title="Heat Score (0-100): tổng hợp Cycle + V/T + YoY" style="text-align:center">Heat</th>
  <th title="Vị trí chu kỳ: thấp = đầu chu kỳ, cao = đỉnh" style="text-align:center">Cycle</th>
  <th title="Nhà Riêng: Giá/m² · ΔGiá 7 ngày · YoY% · Cắt Lỗ%">🏠 Nhà Riêng</th>
  <th title="Chung Cư: Giá/m² · ΔGiá (— nếu chưa crawl) · YoY% · Cắt Lỗ%">🏢 Chung Cư</th>
  <th title="Views/Tin NR và CC — 🔥≥12 ⚡≥7 ❄️<3" style="text-align:center">Thanh Khoản</th>
  <th title="Tín hiệu đầu tư theo Heat Score" style="text-align:center">Tín Hiệu</th>
</tr></thead>
<tbody>${rows}</tbody>
</table>`;
            ...
            document.getElementById('diag-market').innerHTML = rows
                ...
                : '<div style="color:var(--text-3);font-size:12px;padding:8px">Chưa có dữ liệu.</div>';
        };
...
${i + 1}. ${a.name}
                ...
                console.error('Lỗi trong hàm openSimulator:', e);
        ...
        window._SIM_SELL_FLAGS = {};
        function toggleSimSellAsset(idx, isChecked) {
            window._SIM_SELL_FLAGS[idx] = isChecked;
            // Update the UI of the asset card to reflect the selection
            selectSimAsset(SIM_ASSET_IDX);
        }
        ...
        function selectSimAsset(idx) {
            ...
            if (portfolio && portfolio.length > idx) {

                const el = document.getElementById('sim-selected-asset-card');
                if (el) {
                    el.innerHTML = generateAssetCardHTML(portfolio[idx], idx, true);
                    const isSold = window._SIM_SELL_FLAGS[idx];
                    el.innerHTML += `
                        <div style="margin-top: 12px; padding: 12px; background: ${isSold ? 'rgba(239,68,68,0.1)' : 'rgba(255,255,255,0.03)'}; border: 1px solid ${isSold ? 'rgba(239,68,68,0.5)' : 'rgba(255,255,255,0.1)'}; border-radius: 8px; display: flex; align-items: center; justify-content: space-between; transition: all 0.3s">
                            <div style="display: flex; align-items: center; gap: 8px;">
                                <input type="checkbox" id="sim-sell-chk" onchange="toggleSimSellAsset(${idx}, this.checked)" ${isSold ? 'checked' : ''} style="width: 18px; height: 18px; accent-color: var(--red); cursor: pointer;">
                                <label for="sim-sell-chk" style="color: ${isSold ? 'var(--red)' : 'var(--text-2)'}; font-weight: 700; cursor: pointer; margin: 0; font-size: 13px;">🚨 BÁN NGAY TÀI SẢN NÀY</label>
                            </div>
                            ${isSold ? `<span style="font-size: 11px; color: var(--red); font-weight: bold">Đã đưa vào kịch bản bán</span>` : ''}
                        </div>
                    `;
                }
                    ...
                    calc: (inputs) => {
                        ...
                        Object.keys(window._SIM_SELL_FLAGS || {}).forEach(k => {
                            if (window._SIM_SELL_FLAGS[k]) {
                                ...
                                const netProc = sellPrice - debtTy - taxFee - penFee;
                                
                                totalSellProc += netProc;
                                totalSellDebt += debtTy;
                                totalSellCF += (c.cashflow || 0); // They lose this cashflow
                            }
                        });
                        ...
                        // Net impacts
                        const netCapNeeded = totalBuyCapNeeded - totalSellProc;
                        const netDebt = totalBuyDebt - totalSellDebt;
                        const netCF = totalBuyCF - totalSellCF;
                        
                        const isSurplus = netCapNeeded <= 0;
                        const capText = isSurplus ? Math.abs(netCapNeeded).toFixed(2) + ' Tỷ' : netCapNeeded.toFixed(2) + ' Tỷ';
                        const capLabel = isSurplus ? 'Dư Tiền Mặt Sau Giao Dịch' : 'Vốn Tự Có Cần Bơm Thêm';
                        const capColor = isSurplus ? 'var(--emerald)' : 'var(--yellow)';
                        
                        const debtText = netDebt > 0 ? '+' + netDebt.toFixed(2) + ' Tỷ' : netDebt.toFixed(2) + ' Tỷ';
                        const debtColor = netDebt <= 0 ? 'var(--emerald)' : 'var(--red)';
...
${(sc.inputs || []).map(inp => {
                ...
                if (inp.type === 'range') {
                    ...
                    return "<div class='form-group'>" +
                        ...
                        "<label class='form-label' style='margin:0'>" + inp.label + "</label>" +

                        "<span id='val-" + inp.id + "' style='font-size:12px;font-family:monospace;color:var(--gold);font-weight:700'>" + (inp.id === 'sd-capex' && inp.def >= 1000 ? (inp.def / 1000).toFixed(2) + ' Tỷ' : inp.def + (inp.unit || '')) + "</span>" +

                        "</div>" +

                        "<input type='range' step='" + inp.step + "' value='" + inp.def + "' min='" + inp.min + "' max='" + inp.max + "' class='form-input' id='" + inp.id + "' oninput=\"document.getElementById('val-" + inp.id + "').textContent=(this.id === 'sd-capex' && this.value >= 1000 ? (this.value/1000).toFixed(2) + ' Tỷ' : this.value + '" + (inp.unit || '') + "'); runAllScenarios()\" style='padding:0;height:4px;accent-color:" + sc.color + "'>" +
                    ...
                    return "<div class='form-group' style='display:flex;align-items:center;gap:8px'>" +
                        ...
                        "<input type='checkbox' id='" + inp.id + "' " + (inp.def ? "checked" : "") + " onchange='runAllScenarios()' style='accent-color:" + sc.color + ";width:16px;height:16px'>" +

                        "<label class='form-label' style='margin:0;cursor:pointer' for='" + inp.id + "'>" + inp.label + "</label>" +
                    ...
                    return "<div class='form-group'>" +
                        ...
                        "<input type='" + inp.type + "' step='" + inp.step + "' value='" + inp.def + "' class='form-input' id='" + inp.id + "' oninput='runAllScenarios()'>" +
            ...
            SCENARIOS.forEach(sc => {
                ...
                (sc.inputs || []).forEach(inp => {
                    ...
                    if (el) {
                        ...
                        if (inp.type === 'checkbox') inputs[inp.id] = el.checked;
...
<span style="font-family:var(--mono);font-size:12px;font-weight:600;color:${r.color}">${r.val}</span>
        ...
        function renderCompareTable(results) {
            ...
            const c = window._SIM_CALC;

            // 0. Compact asset label in rec badge
...
💡 <strong style="color:var(--text-1)">Hướng dẫn:</strong> So sánh từng cột <strong>Kịch Bản</strong> với cột <strong>HIỆN TẠI (📌)</strong> để đưa ra quyết định. &nbsp; <button class="btn btn-secondary btn-sm" style="font-size:10px" onclick="document.getElementById('sim-asset-list').scrollIntoView({behavior:'smooth',block:'center'})">🔄 Đổi Tài Sản ↑</button>
            ...
            results.forEach(res => {
                ...
                const rowDscr = res.rows.find(r => r.label === 'DSCR');

                const rowCF = res.rows.find(r => r.label === 'Cashflow Ròng / Tháng');

                const valDscr = rowDscr && rowDscr.rawVal !== undefined ? parseFloat(rowDscr.rawVal) : 0;

                const valCF = rowCF && rowCF.rawVal !== undefined ? parseFloat(rowCF.rawVal) : -9999;

                if (valDscr >= 1.0) {

                    safeOptions.push({ res, valCF });
            ...
            if (safeOptions.length > 0) {
                ...
                // Find best CF among safe options

                safeOptions.sort((x, y) => y.valCF - x.valCF);

                const top = safeOptions[0].res;

                recoHtml = `<div style="margin-top:16px;padding:14px;background:rgba(212,175,55,0.1);border:1px solid var(--gold);border-radius:var(--r-md)">

          <div style="font-size:12px;color:var(--gold);font-weight:700;margin-bottom:6px">🏆 KHUYẾN NGHỊ NỔI BẬT: ${top.sc.icon} ${top.sc.title}</div>

          <div style="font-size:11px;color:var(--text-1)">Kịch bản này đưa DSCR về ngưỡng an toàn (>= 1.0) với dòng tiền tốt nhất.</div>

        </div>`;

            } else if (bestSc) {

                recoHtml = `<div style="margin-top:16px;padding:14px;background:rgba(239,68,68,0.1);border:1px dashed var(--red);border-radius:var(--r-md)">

          <div style="font-size:12px;color:var(--red);font-weight:700;margin-bottom:6px">⚠️ VẪN NGUY HIỂM: ${bestSc.sc.icon} ${bestSc.sc.title}</div>

          <div style="font-size:11px;color:var(--text-1)">Tất cả kịch bản đều rủi ro (DSCR < 1.0). Kịch bản tối ưu nhất hiện tại được đề nghị. Hãy cân nhắc phương án Exit Plan để giải tỏa áp lực vốn.</div>
            ...
            tmpDiv.innerHTML = recoHtml;

            body.appendChild(tmpDiv.firstElementChild || tmpDiv);
        ...
        function clearForm() {
            ...
            setTimeout(() => {
                ...
                document.querySelectorAll('.range-slider').forEach(sl => {

                    if (typeof _sliderSync === 'function') _sliderSync(sl);
        ...
        function _initSlidersOnLoad() {
            ...
            [['sl-loanpct', 'f-loanpct'], ['sl-cn-loanpct', 'cn-loanpct'], ['sl-rate', 'f-rate'], ['sl-cn-rate', 'cn-rate'], ['sl-qs-buy', 'qs-buy'], ['sl-qs-now', 'qs-now'], ['sl-qs-loan', 'qs-loan'], ['sl-prefmonths', 'f-prefmonths'], ['sl-cn-prefmonths', 'cn-prefmonths'], ['sl-area', 'f-area'], ['sl-cost', 'f-cost'], ['sl-cn-price', 'cn-price'], ['sl-market', 'f-market'], ['sl-floatrate', 'f-floatrate'], ['sl-cn-floatrate', 'cn-floatrate'], ['sl-grace', 'f-grace'], ['sl-cn-grace', 'cn-grace'], ['sl-loanterm', 'f-loanterm'], ['sl-cn-loanterm', 'cn-loanterm'], ['sl-rent', 'f-rent'], ['sl-mgmt', 'f-mgmt'], ['sl-maint', 'f-maint']].forEach(function (p) {
        ...
        // ── AUTO SUGGEST MARKET PRICE ──────────────────────────────
        function suggestMarketPrice(force) {
            var fmarket = document.getElementById('f-market');
            if (!fmarket) return;
            if (!force && fmarket.value !== "") return; // Only auto-fill if empty

            var hist = window.MARKET_HISTORY;
            if (!hist) {
                if (force) alert("⚠️ Chưa có dữ liệu lịch sử giá (Chạy script inject_history.py)");
                return;
            }

            var ftype = document.getElementById('f-type')?.value;
            var fdist = document.getElementById('f-district')?.value;
            var fmonth = document.getElementById('f-month')?.value;
            var fyear = document.getElementById('f-year')?.value;
            var cost = parseFloat(document.getElementById('f-cost')?.value);

            if (!fdist || !fmonth || !fyear || isNaN(cost) || cost <= 0) {
                if (force) alert("⚠️ Vui lòng nhập đủ: Vị trí, Thời điểm mua và Giá vốn.");
                return;
            }

            var cate = (ftype === 'chung-cu') ? 'chung-cu' : 'nha-rieng';
            var distData = hist[cate] && hist[cate][fdist];
            
            if (!distData) {
                if (force) alert("⚠️ Không có dữ liệu lịch sử cho khu vực này.");
                return;
            }

            var t_buy = 'T' + fmonth + '/' + fyear.toString().slice(-2);
            var p_buy = distData[t_buy];

            var keys = Object.keys(distData);
            if (!keys.length) return;
            
            // Sort keys chronologically (T3/21, T4/21...)
            var sortedKeys = keys.sort((a,b) => {
                var pA = a.split('/'); var pB = b.split('/');
                var mA = parseInt(pA[0].substring(1)), yA = parseInt(pA[1]);
                var mB = parseInt(pB[0].substring(1)), yB = parseInt(pB[1]);
                if (yA !== yB) return yA - yB;
                return mA - mB;
            });
            var p_now = distData[sortedKeys[sortedKeys.length - 1]];

            if (!p_buy || !p_now) {
                if (force) alert("⚠️ Không tìm thấy giá chuẩn tại thời điểm mua (" + t_buy + ").");
                return;
            }

            var ratio = p_now / p_buy;
            var suggestedPrice = cost * ratio;

            fmarket.value = suggestedPrice.toFixed(1);
            
            var slmarket = document.getElementById('sl-market');
            if (slmarket) {
                slmarket.value = fmarket.value;
                updateSliderFill(slmarket);
            }
        }
        ...
        document.addEventListener('DOMContentLoaded', function () {
            ...
            // Hook for suggestMarketPrice
            ['f-cost', 'f-type', 'f-district', 'f-month', 'f-year'].forEach(function (id) {
                var el = document.getElementById(id);
            ...
            window._STRATEGY_CART = savedCart ? JSON.parse(savedCart) : {};
            if (document.getElementById('cart-badge')) {
                document.getElementById('cart-badge').textContent = Object.keys(window._STRATEGY_CART).length;
            }
        } catch(e) {
            window._STRATEGY_CART = {};
        }
        function saveStrategyCart() {
            localStorage.setItem('aa_strategy_cart', JSON.stringify(window._STRATEGY_CART));
        }
        ...
        function editCartItem(id) {
            ...
            const it = window._STRATEGY_CART[id];

            if (!it || !it.isBuyNew) return;

            window._EDITING_CART_ID = id;

            const a = it.asset;

            const setInp = (elemId, val) => {
            ...
            document.getElementById('btn-show-buy').style.display = 'none';

            document.getElementById('cart-modal').scrollTo(0, 0); // scroll to top
        ...
        function fillHotDeal() {
            ...
            const setInp = (id, val) => {
                ...
                if (el) {
                    ...
                    el.dispatchEvent(new Event('input', { bubbles: true })); // syncs sliders automatically
        ...
        function addBuyNewToCart() {
            ...
            const newId = window._EDITING_CART_ID || ('buy-' + Date.now());
            ...
            const aNew = {
                ...
                rent: noi, // Assume NOI approximates rent here for calcAsset logic downstream
            ...
            const cNew = calcAsset(aNew);

            const debtTy = price * loanpct / 100;

            const capNeeded = price - debtTy;
            ...
            window._STRATEGY_CART[sId] = {
                ...
                debtTy: debtTy, // legacy support 

                cf: cNew.cashflow,

                capNeeded: capNeeded,

                asset: aNew // Forward full structured asset to virtual portfolio compiler
        ...
        function renderCartModal() {
          ...
          <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:10px">
            ...
            <div style="display:flex;gap:4px">
              ...
              <button class="btn btn-sm" style="color:var(--yellow);background:rgba(234,179,8,0.1);padding:4px 10px;border-radius:6px;border:none" onclick="editCartItem('${it.id}')">Sửa ✎</button>
          ...
          <div style="font-family:var(--mono);font-size:11.5px;display:grid;grid-template-columns:1fr 1fr;gap:10px">
            ...
            <div style="background:rgba(0,0,0,0.3);padding:8px;border-radius:6px"><span style="color:var(--text-3);display:block;margin-bottom:2px;font-size:10px">Giá Tiền</span> <strong style="color:var(--emerald);font-size:13px">${it.price.toFixed(2)} Tỷ</strong></div><div style="background:rgba(0,0,0,0.3);padding:8px;border-radius:6px"><span style="color:var(--text-3);display:block;margin-bottom:2px;font-size:10px">Diện Tích</span> <strong style="color:var(--emerald);font-size:13px">${it.asset.area || 0} m²</strong></div><div style="background:rgba(0,0,0,0.3);padding:8px;border-radius:6px"><span style="color:var(--text-3);display:block;margin-bottom:2px;font-size:10px">Đơn Giá</span> <strong style="color:var(--yellow);font-size:13px">${it.asset.area ? ((it.price * 1000) / it.asset.area).toFixed(1) : 0} Tr/m²</strong></div><div style="background:rgba(0,0,0,0.3);padding:8px;border-radius:6px"><span style="color:var(--text-3);display:block;margin-bottom:2px;font-size:10px">Tỉ Lệ Vay</span> <strong style="color:var(--text-2);font-size:13px">${it.asset.loanpct || 0}%</strong></div>
                ...
                const navRow = it.rows.find(r => r.label === 'Tiền Mặt Cầm Về / NAV');

                const cfRow = it.rows.find(r => r.label === 'Cashflow Ròng / Tháng');

                const newNav = navRow && navRow.rawVal !== undefined ? parseFloat(navRow.rawVal) : 0;

                const oldNav = it.asset.market - (it.calc.autoDebt / 1000);

                const deltaNav = newNav - oldNav;

                const newCf = cfRow && cfRow.rawVal !== undefined ? parseFloat(cfRow.rawVal) : 0;
                ...
                if (it.scenario.id === 'ban-ngay') {

                    capTriệu = newNav * 1000;

                } else if (it.scenario.id === 'refi') {

                    capTriệu = -(parseFloat(it.inputs['sc-fee']) || 0);

                } else if (it.scenario.id === 'capex') {

                    capTriệu = -(parseFloat(it.inputs['sd-capex']) || 0);

                } else if (it.scenario.id === 'stress') {
        ...
        <div style="font-family:var(--mono);font-size:11.5px;display:grid;grid-template-columns:1fr 1fr;gap:10px">
          ...
          <div style="background:rgba(0,0,0,0.3);padding:8px;border-radius:6px"><span style="color:var(--text-3);display:block;margin-bottom:2px;font-size:10px">Delta Cashflow</span> <strong style="color:${deltaCf >= 0 ? 'var(--emerald)' : 'var(--red)'};font-size:13px">${deltaCf >= 0 ? '+' : ''}${deltaCf.toFixed(1)} Tr</strong></div>
            ...
            const capLabel = totalCapNeeded >= 0 ? 'Tổng Tiền Mặt Giải Phóng' : 'Tổng Phụ Phí / Vốn Đã Đổ';
      ...
      <button class="btn btn-primary btn-sm" style="width:100%;margin-top:12px" onclick="window._IS_VIRTUAL_MODE=true;toggleCartModal();switchTab('prescription');buildPrescription()">📋 Xem Báo Cáo Sau Cơ Cấu →</button>
