...
function calcAsset(a) {
  ...
  const debtB  = (a.debt  || 0) * 1000; // đổi Tỷ → Triệu



  // effectiveRate: tu dong chon rate phu hop
  // prefmonths > 0 = con uu dai -> dung rate uu dai
  // prefmonths = 0 = het uu dai -> dung floatrate (lai tha noi)
  const effectiveRate = (a.prefmonths || 0) > 0
    ? (a.rate || 0)
    : (a.floatrate || a.rate || 0);
  ...
  // BUG2 FIX: sau ân hạn, số tháng còn lại = term × 12 − grace months
  const n_remain  = Math.max(1, n_m - (a.grace||0));
  const principal = a.grace > 0 ? 0 : (n_remain > 0 ? debtB*rateM/(1-Math.pow(1+rateM,-n_remain)) - intOnly : 0);
  ...
  const roeAnnual= equity > 0 ? (cashflow * 12) / (equity * 1000) * 100 : 0;
  ...
  // BUG1 FIX: lãi chỉ tính trên phần vốn vay (debt), không phải toàn bộ cost
  const loanAmt = (a.debt||0) > 0 ? (a.debt||0) : a.cost*(a.loanpct||0)/100;
  const laiCD  = loanAmt * effectiveRate/100 * years;



  const floorPrice = a.cost + laiCD + a.cost*0.02 + a.cost*0.015;
  ...
  const roeScore   = Math.max(0, Math.min(roeAnnual / 18 * 100, 100));
  ...
  const health     = Math.round(dscrScore*0.4 + roeScore*0.4 + vtScore*0.2);
...
function renderDiagnosis(portfolio) {
  ...
  calcs.forEach(({a, c}) => {
    ...
    if (c.dscr !== null && c.dscr < 0.3)
      ...
      alerts.push({ type:'danger', icon:'🚨', msg:`<strong>${a.name}</strong>: DSCR = ${c.dscr.toFixed(2)} — Dòng tiền sắp đứt gãy. Nguy hiểm cao.` });
    ...
    if ((a.grace||0) > 0 && (a.grace||0) <= 3)
      ...
      alerts.push({ type:'warn', icon:'💣', msg:`<strong>${a.name}</strong>: Chỉ còn <strong>${a.grace} tháng</strong> ân hạn nợ gốc. Dòng tiền sắp tăng thêm ${c.principal.toFixed(1)} Tr/th.` });
    ...
    if ((a.prefmonths||0) > 0 && (a.prefmonths||0) <= 3)
      ...
      alerts.push({ type:'warn', icon:'⚡', msg:`<strong>${a.name}</strong>: Lãi ưu đãi hết trong <strong>${a.prefmonths} tháng</strong> — Lãi sẽ nhảy từ ${a.rate}% lên ${a.floatrate}%/năm.` });
  ...
  // Phân tích chiến lược không phù hợp (Strategy Mismatch Warnings)



  const mismatches = calcs.filter(({c}) => c.phaseWarning);
  ...
  if (mismatches.length > 0) {
    ...
    const mBox = mismatches.map(({a,c}) =>



      `<div class="alert alert-warn" style="margin-bottom:8px"><span class="alert-icon">🚧</span><span><strong>${a.name} (Pha ${c.phase}):</strong> ${c.phaseWarning}</span></div>`
    ...
    document.getElementById('diag-alerts').innerHTML += mBox;
...
function getFormData() {
  ...
  return {
    ...
    area: n('f-area'), year: n('f-year'), cost: n('f-cost'),
    ...
    loanpct: n('f-loanpct'), debt: n('f-debt'), rate: n('f-rate'),
...
function _initSlidersOnLoad() {
  [['sl-loanpct','f-loanpct'],['sl-rate','f-rate'],['sl-qs-buy','qs-buy'],['sl-qs-now','qs-now'],['sl-qs-loan','qs-loan'],['sl-prefmonths','f-prefmonths'],['sl-area','f-area'],['sl-cost','f-cost'],['sl-market','f-market'],['sl-debt','f-debt'],['sl-floatrate','f-floatrate'],['sl-grace','f-grace'],['sl-loanterm','f-loanterm'],['sl-rent','f-rent'],['sl-mgmt','f-mgmt'],['sl-maint','f-maint']].forEach(function(p) {
